...
public class GraphPlotter extends JPanel {
    ...
    public GraphPlotter(String expression) { //لما تضغط الماوس، بيحفظ النقطة اللي بدأت فيها السحب
        ...
        addMouseMotionListener(new MouseMotionAdapter() {
            public void mouseDragged(MouseEvent e) {// لو سحبت الماوس، بيحسب الفرق، وبيحدث الإزاحة (offset) ويعيد الرسم
        ...
        addMouseWheelListener(e -> { // إذا لفيت العجلة (scroll)، بيغير قيمة zoom ويعيد الرسم
            zoom *= (e.getPreciseWheelRotation() > 0) ? 0.9 : 1.1;
            repaint();
        });
    }
// تحليل البيانات
    private void generateData() { // بتحلل المعادلة وبتحسب قيم y من100 ل -100
                                  // كل x بزيد ب 0.1 عشان الرسم يكون ناعم
                                  // اذا صار خطأ مثل (0/1) بحط NaN
        xValues.clear();
        yValues.clear();
    ...
    @Override
    protected void paintComponent(Graphics g) { // لما يرسم بستدعي drawghraph
        super.paintComponent(g);
        drawGraph((Graphics2D) g);
    }

    private void drawGraph(Graphics2D g2) { // بيحسب مركز الشاشة (الأصل X=0, Y=0)، ويضيف الإزاحة حسب حركة المستخدم.
        int width = getWidth();
        ...
        // رسم الشبكة
        g2.setColor(Color.LIGHT_GRAY); // بخلي الخلفية بيضا
                                       // بيرسم خطوط شبكة رأسية وأفقية كل مسافة zoom
